...
1. derive the upstream selector that fixes the observable/readout projector and, if possible, the auxiliary low-occupancy rule
2. sharpen the parent octonionic geometry and its reduction to the effective `Spin(2,3)` branch
3. formalize the folding of the exploratory `Spin(3,3)` lift into hidden quaternionic complex-plane data
...
6. keep the static branch paused at conditional closure unless new work bears directly on the upstream selector
